...
void insertionFixup(BST *t, BSTNODE *n)
    ...
    while(1)
        ...
        int zHeight = 0;
        if (z != 0) { zHeight = getAVLVALUEheight(z); }
...
void deletionFixup(BST *t, BSTNODE *n) //TODO FIX UP
    ...
    while(1)
        ...
        if (sib == 0 || xheight > getAVLVALUEheight(z))  //parent favors n
            {
            printf("parent favors\n");
            updateBalance(par);
            n = par;
            continue;
            }
